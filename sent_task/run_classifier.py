...
class DataProcessor(object):
    ...
    def __init__(self, sent_length, tokenizer, true_dir, false_dir):
        self.sent_length = sent_length
        self.tokenizer = tokenizer
        self.data = []
        self.next_idx = 0
        with codecs.open(true_dir, 'r', encoding='utf-8') as f:
            for line in f:
                line = line.strip() + ' && ' + '0'
                self.data.append(line)

        with codecs.open(false_dir, 'r', encoding='utf-8') as f:
            for line in f:
                line = line.strip() + ' && ' + '1'
                self.data.append(line)
    ...
    def get_train_examples(self, batch_size):
        return self._create_example(batch_size)
    ...
    def _create_example(self, batch_size):
        ids = []
        masks = []
        sents_length = []
        labels = []

        if batch_size + self.next_idx > len(self.data):
            self.next_idx = 0
            self.data = shuffle(self.data)
        data = self.data[self.next_idx: self.next_idx + batch_size]
...
def main():
    ...
    processor = DataProcessor(sent_length=sent_length, tokenizer=tokenizer, true_dir=true_file, false_dir=false_file)

    if train:
        num_data = processor.get_num_data()
        ...
        with tf.Graph().as_default() as global_graph:
            ...
            with tf.Session(graph=global_graph, config=config) as sess:
                ...
                for e in range(num_epoch):
                    ...
                    for step in tq:
                        ids, masks, sents_length, labels = processor.get_train_examples(batch_size)
                        ...
                        if step % 100 == 0:
                            ...
                            summary_writter.add_summary(summary, global_step)
